...
class ExHonDashboard {
    ...
    updateProcessesDisplay(processesData) {
        ...
        // Update stats
        this.elements.totalProcesses.textContent = processesData.count || allProcesses.length;
    ...
    showConfirmationModal(action) {
        ...
        document.querySelector('#confirmModal .modal-title').textContent = config.title;
    ...
    async killAll() {
        try {
            ...
            if (result.success) {
                this.showAlert('Successfully terminated all targeted applications and tabs', 'success');
    ...
    async killAI() {
        try {
            ...
            if (result.success) {
                ...
                this.showAlert(`Successfully terminated ${killedCount} AI applications`, 'success');
                this.logActivity(`AI applications terminated: ${killedCount}`, 'success');
            } else {
                this.showAlert('Error terminating AI applications: ' + result.error, 'danger');
            }
        } catch (error) {
    ...
    async closeTabs() {
        try {
            ...
            if (result.success) {
                ...
                this.showAlert(`Successfully closed ${closedCount} browser tabs`, 'success');
    ...
    handleKillCompleted(data) {
        ...
        this.logActivity(`Kill All completed - AI apps: ${aiKilled}, Tabs: ${tabsClosed}`, 'success');
        this.requestUpdate();
    }
